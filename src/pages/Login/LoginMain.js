...
import { loginUserUniversityMain } from "../../pages/Login/loginSlice.js";
import { unwrapResult } from "@reduxjs/toolkit";
import { getTokenSelector, getErrorAuthSelector } from "../../redux/selector";
...
function LoginMain() {
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      const actionResult = await dispatch(loginUserUniversityMain(loginForm));
      const { data } = unwrapResult(actionResult);
      history.push("/app/dashboard-main");
    } catch (error) {
      console.log(error);
              ...
              <form onSubmit={handleOnSubmit}>
                ...
                <p className="mt-4">
                  <Link
                    ...
                    to="/login"
                  >
                    Đăng nhập với các trường thành viên?
                  </Link>
                </p>
              </form>
            </div>
